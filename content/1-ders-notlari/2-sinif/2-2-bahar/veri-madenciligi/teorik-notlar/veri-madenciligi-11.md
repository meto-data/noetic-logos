...
> Gelgelelim zaman serilerinde veriler **RASTGELE SEÇİLEMEZ!** Verinin kronolojik/sıralı olarak eğitilmesi zarurîdir. Zira verilerin sırasını bozarsak, geçmişin geleceği etkileme örüntüsünü yok etmiş oluruz.
...
2. **Mevsimsellik (Seasonality)**: Bir dönem boyunca periyodik olarak tekrar eden belirli bir örüntüyü tanımlar.
...
Zaman serisi tahmininde en büyük problem **bağımsız değişkenlerin ($X$ sütunlarının) olmamasıdır**. Geleneksel makine öğrenmesinde hastanın yaşı, kilosu, şekeri ($X$'ler) verilip hasta olup olmadığı ($Y$) tahmin edilir. Gelgelelim zaman serisinde elimizde yalnızca kronolojik olarak dizilmiş tek bir veri sütunu (örn: elektrik tüketimi) vardır.
