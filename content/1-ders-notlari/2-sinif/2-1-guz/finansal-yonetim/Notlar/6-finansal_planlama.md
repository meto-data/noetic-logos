...
- Bkz. [[Bütçe]], [[Bütçeleme]].
##### Süreç Sıralaması
- Amaç ve Hedefler $\to$ Uzun Vadeli Planlar $\to$ Uzun Vadeli Satış Tahminleri $\to$ Kısa Vadeli Satış Tahminleri $\to$ Bütçelenmiş Mâlî Tablolar 
...
**Not**: Eğer planlanan satışlar mevcut varlıklarla karşılanamıyorsa, [[Sermaye|sermaye]] ve [[nakit]] bütçesi hazırlanarak [[Finansman|finansman]] belirlenir.
...
- Geçmiş satış verileri (*y*) ile zaman (*x*) arasındaki ilişkinin doğrusal bir fonksiyon ($y= a+bx$) ile ifade edilmesidir. Amaçlanan geçmiş verilerdeki sapmaları en aza indiren doğruyu bulmaktır.

$$
\large
\begin{array}{l}
\sum Y_i = n \cdot a + b \cdot \sum X_i \\ \\
\sum (X_i \cdot Y_i) = a \cdot \sum X_i + b \cdot \sum X_i^2
\end{array}
$$
...
\begin{array}{}
E(X) = \frac{K + 4M +I}{6} \\
\normalsize (K: \text{Kötümser tahmin, } M: \text{Muhtemel tahmin, } I: \text{İyimser tahmin})
\end{array}
$$
...
- Gelecekteki belirli bir tarihte işletmenin mâlî durumunu (bilanço) ve faaliyet sonuçlarını (gelir tablosu) tahmin etmek amacıyla hazırlanan tablolardır.
	- "**[[Proforma]]**": Şeklen, usûlen.
- **Kabaca şöyle bir mantık:** Satıştan maliyeti düş $\to$ brüt kâr. Giderleri düş $\to$ faaliyet kârı. Faiz/vergi düş $\to$ net kâr.
...
$$
\large 
\frac{\text{Geçmiş Yıl Net Satışları}}{360} = \text{Günlük Satış Tutarı.}
$$
...
$$
\large 
\frac{\text{Satışa bağlı her bir kalemin tutarı (kasa, stok, alacak vb.)}}{\text{Günlük satış tutarı}} = \text{Katsayı}
$$
...
$$
\large
\frac{\text{Tahmin Edilen Yıllık Satış}}{360} = \text{Tahmini Günlük Satış (TGS)}
$$
...
$$
\large
\text{Katsayılar} \times \text{TGS} = \text{ Kalemin Tahmini Değeri}
$$
...
$$
\large
\text{Tahmini Aktif Toplamı - Tahmini Pasif Toplamı}
$$
